...
public class Sprint2UI extends JFrame {
    ...
    protected JCheckBox recordGame;
    protected JButton startButton;
    protected RecordGame recorder = new RecordGame();
    ...
    public Sprint2UI() {
        ...
        add(recordGame, BorderLayout.SOUTH);
        recordGame.setHorizontalAlignment(SwingConstants.CENTER);
        ...
        addWindowListener(new WindowAdapter() {
            ...
            public void windowClosing(WindowEvent e) {
                recorder.clearRecordedGame();  // Clear the recorded game data
                try {
                    recorder.close();  // Ensure resources are closed
                } catch (Exception ex) {
                    ex.printStackTrace();
                }
    ...
    protected void takeTurn(JButton button) {
        ...
        if (recordGame.isSelected()) {
            if (isBlueTurn) {
                RecordGame.recordMove(row, col, symbol,"blue");
            }

            if (!isBlueTurn) {
                RecordGame.recordMove(row, col, symbol,"red");
